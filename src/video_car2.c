...
void yuv2rgb(char *pbuffer,unsigned int *lcd_mp)
{
    int i,j;
    //用于记录转换完成的ARGB像素点
    unsigned int lcdbuf[IMAGE_WIDTH*IMAGE_HEIGHT] = {0};

    for(i=0,j=0;j<IMAGE_WIDTH*IMAGE_HEIGHT;i+=4,j+=2)
    {
        //                             Y0          U0        V0
    ...
    for (size_t y = 0; y < IMAGE_HEIGHT; y++)
    {
        ...
        {
           lcd_mp[y*800+x] = lcdbuf[y*IMAGE_WIDTH+x];
        }    
    }
