    ...
    //先改标志位再广播
    /*
    那现在情况是这样，有三层判断，第一层lvgl：识别到点击按钮，给按钮功能启动发信号
    第二层按钮结构体启动：根据坐标识别到点击了此位置按钮，改变按钮结构体里的对应标志位，并发送信号
    第三层按钮功能代码段：接收到信号且判断到此标志位启动，执行功能
    那么我是第一层用单独信号量通知第二层、第二层用广播通知第三层
    */
    ...
    第三层按钮功能代码段：接收到信号且判断到此标志位启动，执行功能
    那么我是第一层用单独信号量通知第二层、第二层用广播通知第三层
    //应该把按钮结构体使用标志位控制功能线程与lvgl融合，且不用坐标判断
    持续的功能也要关闭，且需管理共用的，持续的用标志位保持

    */
//把某些状态设置成标志位还是有必要的，如界面或功能正在进行之类，因为有些按钮要检测
//某些状态进行不同的功能
//在不分先后顺序情况下，若是此按钮管理多个功能，由按钮来开启多个标志位
//可以用一个回调函数管理多个按键功能，例如一个界面的功能共用一个回调函数，传入枚举的值，
//发送该界面信号量，每当点击该界面按键，功能线程对应的界面就接收到信号并执行对应的功能
...
void *child_Realtime_buf(void * a)
    ...
    {  
        ...
        if(s_realtime_video.round_pth==1&&s_realtime_video.round_flgs==1)//显示圆圈
        {
            round1();
        ...
        if(s_realtime_video.line_pth==1)//显示辅助线 
        {
            Line_Car( read_buf);
        }
            pthread_mutex_unlock(&mutex_wr_buf_mp);

        if(s_realtime_video_btn[REALTIME_RETURN_BNT].btn==1||s_realtime_video_btn[EXIT_FILM_BNT].btn==1)
        {
            printf("退出传输buffer\n");
            printf("------%d,%d\n",en0_x,en0_y);
...
for(;;)
    {       
        //当辅助线重复按表示关闭
        if(s_realtime_video_btn[GUIDELINE_BTN].btn==1&&s_realtime_video.line_pth==1)
        {
            s_realtime_video_btn[GUIDELINE_BTN].btn=0;
            en0_clear();
            lcd_line_exit();
        }
        if((s_realtime_video_btn[REALTIME_RETURN_BNT].btn==1||s_realtime_video_btn[EXIT_FILM_BNT].btn==1)&&s_realtime_video.line_pth==1)   
        {
            s_realtime_video_btn[GUIDELINE_BTN].btn=0;
...
void * lcd_round1(void * a)
{
    ...
    for(;;)
    {
        ...
        if( s_realtime_video.round_exit==1)   
        {
            ...
            //关闭整个摄像机,录制不用关
        if((s_realtime_video_btn[REALTIME_RETURN_BNT].btn==1||s_realtime_video_btn[EXIT_FILM_BNT].btn==1)&&s_realtime_video.round_flgs==1)   
        {
            lcd_round1_exit();
        }
        //日常运行
...
void * Realtime_video_key(void * a)
{
    ...
    for(;;)
    {
        ...
        {
            printf("退出key\n");
            if( s_realtime_video.line_pth==1)
            {  
        ...
        //点击录像按键
        else if(s_realtime_video_btn[RECOER_BTN].btn==1)
        {
            s_realtime_video_btn[RECOER_BTN].btn=0;
                //检测到不在录制状态点击录制
        ...
        else if(s_realtime_video_btn[GUIDELINE_BTN].btn==1)
        {
            s_realtime_video_btn[GUIDELINE_BTN].btn=0;
            if(s_realtime_video.line_pth==0)
            {
                ...
                s_realtime_video.line_pth=1;
            }
            else if(s_realtime_video.line_pth==1)
            {
                pthread_join(thread_line,NULL);
                s_realtime_video.line_pth=0;
            }
        }   
        else if(REALTIME_IMC &&s_v_c_main.video_flgs==1 )
        {
            en0_clear();
            open_mic();
        }
...
//创建保存录像的文件夹，只有一个
    ...
    //重复点击录像关闭
    //REALTIME_RECOER&&s_v_c_main.video_flgs==1&&s_realtime_video.round_flgs==1 
    ...
    //     {       
    //         //当辅助线重复按表示关闭
    //         if(GUIDELINE&&s_v_c_main.video_flgs==1&&s_realtime_video.line_pth==1)
    //         {
    //             en0_clear();
    //             lcd_line_exit();
    //         }
    //         if((EIXT_FILM|| REALTIME_RETURN)&&s_v_c_main.video_flgs==1&&s_realtime_video.line_pth==1)   
    //         {
    //             s_realtime_video.line_pth=0;
    //             lcd_line_exit();
    //         }
    ...
    //         }
    //         //点击录像按键
    //         else if(REALTIME_RECOER &&s_v_c_main.video_flgs==1 )
    //         {
